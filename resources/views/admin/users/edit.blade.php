                    ...
                    <form method="post" action="{{route('admin.users.update',$item->id)}}"
                        ...
                        <fieldset>
                            <div class="form-group">
                                ...
                                <div class="col-md-6">
                                    <input onkeyup="if (/\D/g.test(this.value)) this.value = this.value.replace(/\D/g,'')" type="number" min="000000" max="9999999"  step class="form-control" name="user_code" value="{{@$item->user_code}}"
                                            required>
                                </div>
                            </div>
                        </fieldset>
                        ...
                        <fieldset>
                            <div class="form-group" id="category">
                                <label class="col-sm-2 control-label" >{{__('اصحاب نجمة')}}

                                </label>
                                <div class="col-md-6">
                                    <select id="nagma_ashab_id" class="form-control select "
                                            name="nagma_ashab_id">
                                        <option value="" >{{__('cp.no')}}</option>
                                        @foreach($nagma_ashab as $one)
                                            <option value="{{$one->id}}" @if($item->nagma_ashab_id==$one->id) selected @endif>{{$one->name}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
