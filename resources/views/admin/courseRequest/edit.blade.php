        ...
        <div class="col-md-12">
            ...
            <div class="portlet light bordered">
                ...
                <div class="portlet-body form">
                    <form method="post" action="{{url(app()->getLocale().'/admin/courseRequest/'.@$order->id)}}"
                    ...
                    <th> {{ucwords(__('cp.courses'))}}</th>
                    <th> {{ucwords(__('cp.price'))}}</th>
                                </tr>
                                </thead>
                                <tbody>
                                    <tr class="odd gradeX">
                        <td><span>{{@$order->institute->name}}</span></td>
                            ...
                            <div class="row">
                                <div class="col-md-offset-3 col-md-9">
                                   @if(@$order->status == -1 || @$order->status == 0 || @$order->status == 1) <button type="submit" class="btn green">{{__('cp.submit')}}</button> @endif
                                    <a href="{{url(getLocal().'/admin/courseRequest')}}" class="btn default">{{__('cp.cancel')}}</a>
                                </div>
                            </div>
                        </div>
