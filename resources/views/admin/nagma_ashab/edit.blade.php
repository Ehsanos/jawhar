                ...
                <div class="portlet-title">
                    <div class="caption">
                        ...
                        <span class="caption-subject font-dark sbold uppercase"
                              style="color: #e02222 !important;">{{ucwords(__('cp.edit'))}} {{__('نجمة اصحاب')}}</span>
                    </div>
                </div>
                <div class="portlet-body form">
                    <form method="post" action="{{url(app()->getLocale().'/admin/nagma_ashab/'.$nagma->id)}}"
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-md-6">
                                    <input type="text" class="form-control" name="name"
                                           id="name"
                                           value="{{ old('name', $nagma->name) }}" required>
                                </div>
                            </div>
                        </fieldset>
                                ...
                                <div class="col-md-6">
                                    <select class="form-control select2" id="nagma_ashab_cards_ids" name="nagma_ashab_cards_ids[]" multiple="multiple" required>
                                        @foreach($nagma_ashab_cards_ids as$one)
                                            <option value="{{$one->id}}"
                                            @foreach(explode(',',$nagma->ashab_cards_ids) as $one_id)
                                                @if($one_id ==  $one->id)
                                                        selected
                                                    @break
                                                @endif
                                            @endforeach
                                            >{{$one->card_name}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                        ...
                        <div class="form-actions">
                            <div class="row">
                                <div class="col-md-offset-3 col-md-9">
                                    ...
                                    <a href="{{url(getLocal().'/admin/nagma_ashab')}}" class="btn default">{{__('cp.cancel')}}</a>
                                </div>
                            </div>
                        </div>
                    </form>
