        ...
        <div class="portlet-body">
            <div class="table-toolbar">
                <div class="row">
                    <div class="col-sm-9">
                        <div class="btn-group">
                            <a href="{{url(getLocal().'/admin/stores/'.$user->id.'/createStoreProduct')}}" style="margin-right: 5px" class="btn sbold btn-primary">
            ...
            <table class="table table-striped table-bordered table-hover table-checkable order-column" id="toolsTable1">
                ...
                <tr>
                    ...
                    <th> {{ucwords(__('cp.subCategories'))}}</th>
                    <th> {{ucwords(__('رقم الصنف الفرعي'))}}</th>
                ...
                @forelse(@$products as $product)
                    <tr  class="odd gradeX" id="tr-{{$product->id}}">
                        ...
                        <td>@if($product->is_dollar == 1)
                                {{$product->real_price}} $ <br>
                                ( {{$product->result($product->real_price)}} ) ₺
                            @else
                                {{$product->price}} ₺
                            @endif
                        </td>
